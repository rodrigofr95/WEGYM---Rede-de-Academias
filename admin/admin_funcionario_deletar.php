...
if (!$conn) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        $sql_cpf = "SELECT * FROM usuario INNER JOIN funcionario ON funcionario.idusuario = usuario.id WHERE cpf=? AND status='ativo'";
        ...
        if (mysqli_num_rows($result_cpf) > 0) {
            ...
            $idusuario = isset($row['idusuario']) ? $row['idusuario'] : '';
            $email = $row['email'];
            $perfil = $row['perfil'];
            $status = $row['status'];
            $nome = $row['nome'];
...
<main>
    <div class="jumbotron p-md-5 text-white bg-dark">
        <div class="col-md px-0">
            ...
            <h5 class="font-italic color-white">EXCLUIR CADASTRO DE FUNCIONÁRIO</h5>
        </div>
    </div>

    <div class="container">
        <h1>Insira o CPF do funcionário que deseja excluir:</h1>
        ...
        <?php if (isset($cpf) && !isset($cpfNotFound)) { ?>
            <div class="container">
                <form method="post" action="funcionario_deleta.php">
                    <table class="table table-striped">
                        ...
                        <tr>
                            ...
                            <td><input type="text" name="cargo" value="<?php echo $cargo; ?>" readonly></td>
                        </tr>
                        <tr>
